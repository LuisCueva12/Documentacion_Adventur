...
| RF-19 | Políticas configurables en PDF | `GET/PUT /api/v1/admin/configuracion/politicas` | `ListarPoliticasDocumentoUseCase`, `ActualizarPoliticasDocumentoUseCase`, `ServicioPoliticasDocumento` | `PoliticaDocumentoControlador@index` / `@update` | Cuatro políticas ordenadas, edición versionada, transacción y auditoría por puerto; configuraciones numéricas quedan excluidas |
...
| **Admin/Config** | `PoliticaConfiguracion`, usuarios y roles administrativos | `InterfazRepositorioPoliticaConfiguracion`, `InterfazRepositorioUsuarioAdministracion`, `InterfazRepositorioRolAdministracion` | Casos de uso `*AdministracionUseCase`, `Listar/ActualizarPoliticasDocumentoUseCase`, `ServicioAuditoria` | `UsuarioAdministracionControlador`, `RolAdministracionControlador`, `PoliticaDocumentoControlador` | `src/views/Administracion/PoliticasDocumento.vue`, `UsuariosLista.vue` |
